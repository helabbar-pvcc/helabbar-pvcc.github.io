# Name: Hanan El Abbar
# Prog Purpose: This progeram reads a comman-delimited data file into a LIST of smaller lists.
...
def read_data_file():
    ...
    for i in lines:
        stu.append(i.split(",")) #Split the data by comma and add each section to the lost
    datafile.close()

def calculate_grades ():
    global average, totA, totB, totC, totD, totF
    totA= totB= totC= totF = 0
    gradestotal=0

    for i in range (len(stu)):
        hw= int(stu[i][2])
        midex = int(stu[i][3])
        stu[i][4] = stu[i][4][:-1]   # Remove the \n from the final exam string (end of th einput line)
        finex = int(stu[i][4]) 
...
def create_report_file():
    ...
    repfile.write ("n\*****************CSC 230 Grade Report *****************")
